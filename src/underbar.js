(function() {
  ...
  _.filter = function(collection, test) {
    ...
    _.each(collection, function(item){
      if(test(item)){returner.push(item)}
    })
    return returner;
  };
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection,function(prev,cur){
      ...
      var iterated = iterator (cur);
      if (typeof(iterated) !== 'boolean') {
        if (iterated === 1) {
          iterated = true;
        } else if (iterated === undefined || iterated === null) {
          iterated = 	false;
        } else if (iterated === 0) {
          iterated = false;
        } else {
  ...
  _.extend = function(obj) {
      ...
      for (var k in arguments) {
        for (var ke in arguments[k]) {
          newObj[ke] = arguments[k][ke];
        }
      }
      return newObj;
  };
